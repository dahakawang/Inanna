...
class TestCharCodecIO < Test::Unit::TestCase
    ...
    def test_decodeFromFile_utf16le_support
      codec = Inanna::CharCodecIO.new
      data = nil

      assert_nothing_raised(Inanna::EncodingNotDetectError) do
        data = codec.decodeFromFile("test/TextData/sample-utf16le.txt")
      end
      assert_equal "UTF-16LE", codec.last_encoding
      assert_equal STR_CONST, data
    end

    def test_decodeFromFile_utf16be_support
      codec = Inanna::CharCodecIO.new
      data = nil

      assert_nothing_raised(Inanna::EncodingNotDetectError) do
        data = codec.decodeFromFile("test/TextData/sample-utf16be.txt")
      end
    ...
    def test_decodeFromFile_not_supported_encoding
      codec = Inanna::CharCodecIO.new
      data = nil
    ...
    def test_decodeFromFile_and_specify_encoding
      codec = Inanna::CharCodecIO.new
      data = nil

      codec.specifyEncoding("BIG5")
      assert_nothing_raised(Inanna::EncodingNotDetectError) do
        data = codec.decodeFromFile("test/TextData/sample-big5.txt")
      end
